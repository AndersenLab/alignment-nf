...
if [[ $(cut -f 2 ${fq_sheet}  | sort | uniq -c | grep -v '1 ') ]]; then
    echo "There are duplicate IDs in the sample sheet. Please review 'isotype_sheet.error'"
    cat ${fq_sheet} | sort > ../isotype_sheet.error
    exit 1
else
    cat ${fq_sheet} | sort > ../SM_isotype_sheet.tsv
    echo "$(cat ../SM_isotype_sheet.tsv | wc -l) records. Sample sheet output to ../SM_isotype_sheet.tsv"
fi
...
#===================================#
cat ${fq_sheet} | python strain_to_isotype.py | sort > ../SM_isotype_sheet.tsv
